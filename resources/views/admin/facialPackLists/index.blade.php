...
@section('title')
Facial Pack Info List | {{ $ins_name }}
@endsection
        ...
        <div class="row">
            <div class="col-12">
                <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                    <h4 class="mb-sm-0">Facial Pack Info List</h4>

                    <div class="page-title-right">
                        <ol class="breadcrumb m-0">
                            <li class="breadcrumb-item"><a href="javascript: void(0);">Tables</a></li>
                            <li class="breadcrumb-item active">Facial Pack Info List</li>
                        </ol>
                    </div>
            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title mb-0">Facial Pack Info</h4>
...
->where('face_pack_id',$allTherapyLists->id )->get();
                                          ...
                                          @foreach($faceIngredientList as $allFaceIngredientList)
                                                    ...
                                                    $getFinalName = DB::table('other_ingredients')
                                                    ->where('id',$allFaceIngredientList->ingredient_id )->value('name');

                                                      ?>

{{ $getFinalName  }} - {{ $allFaceIngredientList->amount }}<br>
                                        ...
                                        <td>
                                            ...
                                            @if (Auth::guard('admin')->user()->can('facialInfoListUpdate'))
                                            <a type="button" href="{{ route('facialInfoList.edit',$allTherapyLists->id) }}"
  ...
  {{-- <button type="button" class="btn btn-primary waves-light waves-effect  btn-sm" onclick="window.location.href='{{ route('admin.users.view',$user->id) }}'"><i class="fa fa-eye"></i></button> --}}
                                    ...
                                    @if (Auth::guard('admin')->user()->can('facialInfoListDelete'))
  ...
  <form id="delete-form-{{ $allTherapyLists->id }}" action="{{ route('facialInfoList.destroy',$allTherapyLists->id) }}" method="POST" style="display: none;">
    @method('DELETE')
                                  @csrf
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('facialInfoList.store') }}" method="post" enctype="multipart/form-data" id="form" data-parsley-validate="">
                    @csrf
                    <div class="row">
